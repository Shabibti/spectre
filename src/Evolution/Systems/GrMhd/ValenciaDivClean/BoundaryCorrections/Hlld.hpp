 ...
 * the HLLD boundary correction is \cite Harten1983
 *
 * \f{align*}
 * G_\text{HLLD} = \frac{\lambda_\text{max} F_\text{int} +
 ...
 * versa. Similarly, the \f$F_{\text{ext}}\f$ term in \f$G_\text{HLLD}\f$ has a
 ...
 *   (i.e. all characteristics move in the same direction) the HLLD boundary
 ...
 *   those variables.
 */
class Hlld final : public BoundaryCorrection {
 public:
