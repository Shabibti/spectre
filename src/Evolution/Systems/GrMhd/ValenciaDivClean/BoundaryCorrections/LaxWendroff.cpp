...
#include "Evolution/Systems/GrMhd/ValenciaDivClean/BoundaryCorrections/LaxWendroff.hpp"
...
LaxWendroff::LaxWendroff(CkMigrateMessage* /*unused*/) {}

std::unique_ptr<BoundaryCorrection> LaxWendroff::get_clone() const {
  return std::make_unique<LaxWendroff>(*this);
}

void LaxWendroff::pup(PUP::er& p) { BoundaryCorrection::pup(p); }

double LaxWendroff::dg_package_data(
...
void LaxWendroff::dg_boundary_terms(
  ...
  if (dg_formulation == dg::Formulation::WeakInertial) {
    get(*boundary_correction_tilde_d) = get(normal_dot_flux_tilde_d_LW);
    get(*boundary_correction_tilde_ye) = get(normal_dot_flux_tilde_ye_LW);
    get(*boundary_correction_tilde_tau) = get(normal_dot_flux_tilde_tau_LW);
    get(*boundary_correction_tilde_phi) = get(normal_dot_flux_tilde_phi_LW);

    for (size_t i = 0; i < 3; ++i) {
      boundary_correction_tilde_s->get(i) = normal_dot_flux_tilde_s_LW.get(i);
      boundary_correction_tilde_b->get(i) = normal_dot_flux_tilde_b_LW.get(i);
    }
  } else {
    get(*boundary_correction_tilde_d) = get(normal_dot_flux_tilde_d_LW);
    get(*boundary_correction_tilde_ye) = get(normal_dot_flux_tilde_ye_LW);
...
bool operator==(const LaxWendroff& /*lhs*/, const LaxWendroff& /*rhs*/) {
  return true;
}
...
// NOLINTNEXTLINE
PUP::able::PUP_ID LaxWendroff::my_PUP_ID = 0;
}  // namespace grmhd::ValenciaDivClean::BoundaryCorrections
